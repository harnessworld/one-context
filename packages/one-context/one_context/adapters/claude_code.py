...
class ClaudeCodeAdapter(AdapterBase):
    ...
    def generate_agents(
        ...
        Claude Code follows ``@file`` references at runtime, so each agent
        file lists its knowledge as ``@path`` lines.  Users add
        ``@.claude/agents/{id}.md`` to their ``CLAUDE.md`` once — after that
        a single natural-language sentence is all that is needed to activate
        an agent.
        """
