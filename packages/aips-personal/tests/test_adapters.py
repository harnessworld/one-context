...
from aiws.adapters import ADAPTERS, GeneratedFile, get_adapter, list_adapters
...
def adapter_root(tmp_path: Path) -> Path:
    """Create a minimal aips-personal tree with knowledge files."""
    meta = tmp_path / "meta"
    meta.mkdir()
...
class TestClaudeCodeAdapter:
    def test_generates_file(self, adapter_root, dev_context):
        ...
        assert gf.rel_path == ".claude/adapters/aiws-dev.md"
    ...
    def test_content_has_workspace_header(self, adapter_root, dev_context):
        ...
        assert "# aips-personal" in content
...
class TestCursorAdapter:
    def test_generates_mdc_file(self, adapter_root, dev_context):
        ...
        assert len(files) == 1
        gf = files[0]
        assert gf.rel_path == ".cursor/rules/aiws-dev.mdc"
...
class TestOpenClawAdapter:
    def test_generates_json_file(self, adapter_root, dev_context):
        ...
        assert len(files) == 1
        gf = files[0]
        assert gf.rel_path == ".openclaw/aiws-dev.json"
...
class TestAdaptCLI:
    def test_adapt_dry_run(self, adapter_root, capsys):
        from aiws.cli import build_parser, _resolve_root

        parser = build_parser()
        args = parser.parse_args(["--root", str(adapter_root), "adapt", "dev", "--dry-run"])

        from aiws.cli import _cmd_adapt
    ...
    def test_adapt_writes_files(self, adapter_root):
        from aiws.cli import build_parser
        parser = build_parser()
        ...
        assert (adapter_root / ".claude" / "adapters" / "aiws-dev.md").is_file()
        assert (adapter_root / ".cursor" / "rules" / "aiws-dev.mdc").is_file()
        assert (adapter_root / ".openclaw" / "aiws-dev.json").is_file()
    ...
    def test_adapt_only_filter(self, adapter_root):
        ...
        assert (adapter_root / ".cursor" / "rules" / "aiws-dev.mdc").is_file()
        assert not (adapter_root / ".claude" / "adapters" / "aiws-dev.md").is_file()
