# -*- coding: utf-8 -*-
"""烧录字幕 - 通过 bat 文件绕过 PowerShell 路径转义"""
import subprocess, json, re, pathlib, os, tempfile
...
# Write bat file (ANSI/GBK encoded for Windows cmd)
bat_path = os.path.join(tmp_dir, "_burn.bat")
# Build command with properly quoted paths for cmd
# Use ^ to escape quotes in cmd
video_q = '"' + video.replace('"', '^"') + '"'
out_q = '"' + out.replace('"', '^"') + '"'
ass_q = '"' + ass_path.replace('"', '^"') + '"'

bat_content = (
    'chcp 65001 >nul\n'
    'ffmpeg -y -i ' + video_q + ' -vf ass=' + ass_q + ' -c:a copy ' + out_q + '\n'
