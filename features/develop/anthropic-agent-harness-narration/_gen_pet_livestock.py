# -*- coding: utf-8 -*-
"""Anthropic Agent Harness 播客 PPT - 动物主题暖色调
    规格：1080×1920 | 零空白 | 字号加大
    主题：宠物 vs 牲口 — 暖土色 + 动物意象
"""
import os
...
def svg(name, color=None):
    ...
    icons = {
        ...
        "dog":        f'<svg viewBox="0 0 64 64" width="48" height="48" fill="none"><path d="M14 38c0-12 8-20 20-20s20 8 20 20" stroke="{c}" stroke-width="3" fill="none" opacity=".7"/><circle cx="20" cy="32" r="5" fill="{c}" opacity=".6"/><circle cx="44" cy="32" r="5" fill="{c}" opacity=".6"/><path d="M26 40c0 0 3 5 6 5s6-5 6-5" stroke="{c}" stroke-width="2.5" stroke-linecap="round" fill="none" opacity=".8"/><path d="M8 30l6-8M56 30l-6-8" stroke="{c}" stroke-width="3" stroke-linecap="round" opacity=".6"/></svg>',
        "cattle":     f'<svg viewBox="0 0 64 64" width="48" height="48" fill="none"><ellipse cx="32" cy="36" rx="22" ry="14" fill="{c}" opacity=".7"/><circle cx="32" cy="24" r="12" fill="{c}" opacity=".75"/><path d="M20 18c-4-8-2-12 4-10" stroke="{c}" stroke-width="3" stroke-linecap="round" fill="none" opacity=".7"/><path d="M44 18c4-8 2-12-4-10" stroke="{c}" stroke-width="3" stroke-linecap="round" fill="none" opacity=".7"/><path d="M10 36h-4M54 36h4" stroke="{c}" stroke-width="4" stroke-linecap="round" opacity=".5"/></svg>',
        "layers":     f'<svg viewBox="0 0 64 64" width="48" height="48" fill="none"><path d="M32 8L8 20v24l24 12 24-12V20L32 8z" fill="{c}" opacity=".75"/><path d="M32 18L14 28v18l18 9 18-9V28L32 18z" fill="{c}" opacity=".55"/></svg>',
        ...
        "brain":      f'<svg viewBox="0 0 64 64" width="48" height="48" fill="none"><ellipse cx="32" cy="32" rx="22" ry="20" fill="{c}" opacity=".75"/><path d="M20 26c4-4 8-4 12 0M32 26c4-4 8-4 12 0M18 38c4-4 8-4 14 0M32 38c4-4 8-4 12 0" stroke="#1a0f0a" stroke-width="2.5" stroke-linecap="round" opacity=".7"/></svg>',
        "server":     f'<svg viewBox="0 0 64 64" width="48" height="48" fill="none"><rect x="8" y="12" width="48" height="14" rx="4" fill="{c}" opacity=".7"/><rect x="8" y="32" width="48" height="14" rx="4" fill="{c}" opacity=".85"/><rect x="8" y="52" width="48" height="14" rx="4" fill="{c}" opacity=".6"/><circle cx="18" cy="19" r="3" fill="#1a0f0a" opacity=".7"/><circle cx="18" cy="39" r="3" fill="#1a0f0a" opacity=".7"/><circle cx="18" cy="59" r="3" fill="#1a0f0a" opacity=".7"/></svg>',
        "warning":    f'<svg viewBox="0 0 64 64" width="48" height="48" fill="none"><path d="M32 8L4 56h56L32 8z" fill="{c}" opacity=".85"/><rect x="29" y="22" width="6" height="18" rx="3" fill="#1a0f0a" opacity=".8"/><circle cx="32" cy="46" r="3" fill="#1a0f0a" opacity=".8"/></svg>',
        ...
        "cpu":        f'<svg viewBox="0 0 64 64" width="48" height="48" fill="none"><rect x="16" y="16" width="32" height="32" rx="6" fill="{c}" opacity=".8"/><path d="M24 8v8M40 8v8M24 48v8M40 48v8M8 24h8M8 40h8M48 24h8M48 40h8" stroke="{c}" stroke-width="3" stroke-linecap="round" opacity=".6"/><rect x="24" y="24" width="16" height="16" rx="2" fill="#1a0f0a" opacity=".5"/></svg>',
        ...
        "paw_large":  f'<svg viewBox="0 0 80 80" width="96" height="96" fill="none"><ellipse cx="40" cy="52" rx="18" ry="12" fill="{c}" opacity=".75"/><circle cx="22" cy="34" r="8" fill="{c}" opacity=".65"/><circle cx="58" cy="34" r="8" fill="{c}" opacity=".65"/><circle cx="30" cy="20" r="7" fill="{c}" opacity=".6"/><circle cx="50" cy="20" r="7" fill="{c}" opacity=".6"/></svg>',
    ...
    return icons.get(name, icons["paw"])
...
#progress {
    ...
    height: 3px;
    background: linear-gradient(90deg, #c2714f, #d4a843);
    z-index: 999;
...
.timestamp {
    ...
    font-size: 11px; color: rgba(140,122,106,0.4); letter-spacing: 1px;
}
.page-num {
...
.cover-bg {
    ...
    background:
        radial-gradient(ellipse 80% 60% at 50% 30%,
            rgba(194,113,79,0.15) 0%,
            rgba(212,168,67,0.07) 40%,
            transparent 70%),
...
.grid-card p {
    ...
    color: rgba(245,230,211,0.6); line-height: 1.5; white-space: pre-line;
}
...
.big-sub {
    ...
    color: rgba(212,168,67,0.6); letter-spacing: 4px; margin-top: 16px;
}
...
.layer-desc {
    ...
    color: rgba(245,230,211,0.85); line-height: 1.6; white-space: pre-line;
}
...
.contrast-box {
    padding: 28px 48px; border-radius: 20px;
...
.num-label { font-size: clamp(14px, 1.5vw, 18px); opacity: 0.7; margin-top: 10px; letter-spacing: 1px; }

/* ─── 流程 ─── */
.flow { display: flex; flex-direction: column; gap: 14px; width: 100%; margin-top: 28px; }
.flow-step { display: flex; align-items: center; gap: 20px; padding: 22px 28px; border-radius: 16px; background: rgba(212,168,67,0.06); border: 1px solid rgba(212,168,67,0.15); }
.flow-emoji { font-size: 36px; flex-shrink: 0; width: 56px; text-align: center; }
.flow-step-title { font-size: clamp(20px, 2.2vw, 26px); font-weight: 700; color: #d4a843; margin-bottom: 4px; }
.flow-step-desc { font-size: clamp(15px, 1.5vw, 18px); color: rgba(245,230,211,0.7); }
...
.contrast-item { font-size: clamp(15px, 1.5vw, 18px); padding: 6px 0; opacity: 0.85; }
...
.icon-list-item { display: flex; align-items: center; gap: 20px; padding: 20px 24px; border-radius: 16px; background: rgba(245,230,211,0.04); border: 1px solid rgba(245,230,211,0.08); }
.icon-list-icon { width: 44px; height: 44px; flex-shrink: 0; }
.icon-list-icon svg { width: 44px; height: 44px; }
.icon-list-text h4 { font-size: clamp(18px, 2vw, 24px); font-weight: 700; color: #f5e6d3; margin-bottom: 4px; }
.icon-list-text p { font-size: clamp(14px, 1.4vw, 17px); color: rgba(245,230,211,0.6); white-space: pre-line; }

/* ─── 痛点 ─── */
.pain-items { width: 100%; margin-top: 28px; display: flex; flex-direction: column; gap: 14px; }
.pain-item { display: flex; align-items: center; gap: 20px; padding: 22px 28px; border-radius: 16px; background: rgba(194,113,79,0.08); border: 1px solid rgba(194,113,79,0.22); }
.pain-item-icon { font-size: 32px; flex-shrink: 0; width: 44px; text-align: center; }
.pain-item-text { font-size: clamp(22px, 2.8vw, 34px); font-weight: 700; color: #c2714f; }
...
.timeline-num { min-width: 60px; height: 60px; border-radius: 50%; background: linear-gradient(135deg, #c2714f, #d4a843); display: flex; align-items: center; justify-content: center; font-family: 'Consolas', 'Courier New', monospace; font-size: 22px; font-weight: 700; flex-shrink: 0; color: #1a0f0a; }
.timeline-content { flex: 1; padding: 20px 24px; border-radius: 16px; background: rgba(212,168,67,0.06); border: 1px solid rgba(212,168,67,0.15); }
.timeline-title { font-size: clamp(18px, 2vw, 24px); font-weight: 700; color: #d4a843; margin-bottom: 6px; }
.timeline-desc { font-size: clamp(15px, 1.5vw, 18px); color: rgba(245,230,211,0.7); }
...
.resource-desc { font-size: clamp(16px, 1.6vw, 20px); color: rgba(245,230,211,0.8); line-height: 1.6; white-space: pre-line; }

/* ─── 比喻 ─── */
.metaphor-badges { display: flex; gap: 14px; justify-content: center; flex-wrap: wrap; margin-top: 20px; }
.metaphor-badge { padding: 12px 28px; border-radius: 100px; font-size: clamp(16px, 1.8vw, 22px); font-weight: 700; border: 2px solid rgba(212,168,67,0.4); color: #d4a843; background: rgba(212,168,67,0.06); }

/* ─── 结尾 ─── */
.ending-emphasis { font-size: clamp(28px, 3.5vw, 48px); font-weight: 700; color: #f5e6d3; text-align: center; margin-top: 20px; }
.ending-sub { font-size: clamp(22px, 2.5vw, 30px); font-weight: 700; color: #d4a843; text-align: center; margin-top: 20px; padding: 20px 40px; border: 2px solid rgba(212,168,67,0.4); border-radius: 16px; background: rgba(212,168,67,0.08); }
...
def slide_html(s):
  ...
  {pt}{tst}
    ...
    if t == "big_center":
        color = s.get("accent_color", "#d4a843")
  ...
  <div class="contrast-center">
    <div class="contrast-row">
      <div class="contrast-box" style="color:#c2714f;border-color:rgba(194,113,79,0.6);background:rgba(194,113,79,0.1)">{s['before']}</div>
      <div class="contrast-arrow-icon">{ic}</div>
      <div class="contrast-box" style="color:{color};border-color:{hex_alpha(color,0.6)};background:{hex_alpha(color,0.1)}">{s['after']}</div>
    </div>
  </div>
  {pt}{tst}
</div>"""
    ...
    if t == "numbers":
        ...
        for label, val, desc in s.get("numbers", []):
            nums += f'''<div class="num-card" style="border-color:rgba(212,168,67,0.5);background:rgba(212,168,67,0.08)">
  <div class="num-value" style="background:linear-gradient(135deg,#c2714f,#d4a843);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text">{val}</div>
  ...
  <div class="contrast-layout">
    <div class="contrast-col" style="background:rgba(194,113,79,0.07);border-color:rgba(194,113,79,0.22)">
      <div class="contrast-col-title" style="color:#c2714f">{s['before_label']}</div>
      {before_items}
    </div>
    <div class="contrast-arrow-col">
      <div style="width:48px;height:48px;">{svg('arrow')}</div>
    </div>
    <div class="contrast-col" style="background:rgba(125,156,108,0.07);border-color:rgba(125,156,108,0.22)">
      <div class="contrast-col-title" style="color:#7d9c6c">{s['after_label']}</div>
      {after_items}
    </div>
  </div>
  {pt}{tst}
  ...
  <div class="pain-item-text">{item}</div>
        ...
        sub = f'<div class="body-text" style="margin-top:20px;font-size:clamp(18px,2vw,24px);color:rgba(194,113,79,0.7)">{s["sub"]}</div>' if s.get("sub") else ""
        return f"""<div class="slide" id="s{s['id']}">
  <div class="top-bar"></div>
  {ts}{tt}
  ...
  {pt}{tst}
    ...
    if t == "metaphor":
        ...
        metaphor = f'<div style="margin-top:28px;font-size:clamp(28px,3vw,40px);color:#d4a843;font-weight:900">{s["metaphor"]}</div>' if s.get("metaphor") else ""
        return f"""<div class="slide" id="s{s['id']}">
  <div class="top-bar"></div>
  {ts}{tt}
  <div class="body-text">{s.get('body','')}</div>
