...
WAV → 校对 SRT → 拆分话题 → 生成幻灯 → 渲染 MP4（带烧录字幕）的端到端 **`/step` 计划表**。

**定位**：整条流水线的**编排入口**即本仓库内的 **`pipeline/video-pipeline.step.yaml`**（由 **`/step --plan_file …`** 加载）。各步骤里需要成片、对齐、封面时，会调用与 **`SKILL.md`** 相同的 **`node cli.js …`** 与 **`scripts/`**，并非第二套工具链。无 **`/step`** 编排器时不能执行本 YAML，请改读 **`SKILL.md`** 的 cli 子命令逐段跑。

## 工作原理

将此流水线与 **`/step`** 命令配合使用（具体命令名与参数以你本机 step 插件为准）。
...
**Windows**：须能用同一 Python 跑通 Step 1（含 `import faster_whisper` 时 Whisper 才可用）；Step 2/3 的 Whisper / `t2s_srt.py` 使用 **当前解释器 `sys.executable`**，不再强制要求 `python3` 在 PATH。`node`、`ffmpeg` 仍须在 PATH。

### 目录结构要求

素材目录（`PROJECT_DIR`）须能在 **`video-input.json` 的 `wavFile`**（或根目录 / `media/` 扫描）下找到 **口播 WAV**。推荐标准布局：

```
production/
├── timing/video-input.json   # wavFile、srtFile、Whisper 参数、可选 podcastTts
├── media/voiceover.wav       # 口播（可由 Step 1 从讲稿合成）
├── content/01-script.md      # 可选：播客 API 长文输入（action=0 总结双人）
├── subtitles/sub.srt         # Step 2 Whisper 输出（与 srtFile / wav-durations 对齐）
└── slides/presentation.html
```

### 从讲稿自动生成 WAV（火山互动播客，`skills/volc-podcast-tts`）

在 **`timing/video-input.json`**（或根目录同名文件）中配置：

| 字段 | 含义 |
|------|------|
| **`wavFile`** | 合成产物路径（相对 `PROJECT_DIR`），如 `media/voiceover.wav`（启用播客合成时必填） |
| **`srtFile`** | Whisper 输出的 SRT 相对路径；缺省时读取 `timing/wav-durations.json` 的 `srtFile`，再缺省为存在 `subtitles/` 目录时用 `subtitles/sub.srt`，否则 `sub.srt` |
| **`podcastTts.enabled`** | `true` 时 Step 1 在缺失 WAV 或 `forceRegenerate` 时调用火山播客 CLI |
| **`podcastTts.scriptPath`** | 默认 `content/01-script.md` |
| **`podcastTts.action`** | 默认 `0`（长文总结双人播客）；`3` 为对白逐轮直出 |
...
合成成功后 Step 2 仍用 **`align_wav_slides.py --srt-only`** 生成字幕，后续 Step 3–11 **不变**。

**`video-input.json` 合并规则（Step 1）**：先读项目根 `video-input.json`，再读 **`timing/video-input.json`**，后者字段覆盖前者（与 `production/timing/` 为权威一致）。
...
# 3. 运行完整流水线
/step --plan_file skills/html-video-from-slides/pipeline/video-pipeline.step.yaml 端到端视频制作
```

或使用更简洁的方式（Claude Code 会自动解析 `--plan_file`）：

```
/step run --plan_file skills/html-video-from-slides/pipeline/video-pipeline.step.yaml
```
...
```
/step run --plan_file skills/html-video-from-slides/pipeline/video-pipeline.step.yaml --from_step 9
```

## 步骤详解（权威以 `video-pipeline.step.yaml` 为准）

**主干（Step 1–11）**：环境 **[+ 可选 01-script→火山播客 WAV]** → Whisper → 繁简与预处理 →（推理）字幕校对 →（推理）拆页 →（推理）内容板 →（推理）视觉 HTML → 校验 → **`node cli.js wav` 成片**。

| Step | 类型 | 名称 | 用户交互 |
|------|------|------|----------|
| 1–3 | script | 预检 **[+ podcastTts]**、Whisper、`t2s`+预处理 | 无 |
...
| 封面缺失 | 优先：`cover.html` + `node cli.js cover --project <dir>`；若坚持用本流水线尾部则 `--from_step 12`（依赖 `html-deck/` 约定） |
