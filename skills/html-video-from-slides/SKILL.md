...
4. **字幕与稳定性（wav-auto）**：
   ...
   - 也可设 `"subtitle": { "fontSize": 18, "marginV": 18, "fontName": "Microsoft YaHei", "bold": true, "primaryColour": "#FFFF00", "primaryAlpha": 108, "charsPerLine": 42, "barHeight": 80, "backAlpha": 255, "shadow": 0, "outline": 1 }` 自定义样式（默认即黄字偏小字号；`primaryAlpha` 为 ASS 透明度：**0** 不透明，**255** 全透明）。tts 模式始终烧录字幕。
