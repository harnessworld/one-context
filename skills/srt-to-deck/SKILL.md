...
## 口播字幕驱动的内容精修（双人对话 / 播客 / 中视频）

当用户要求 **「按字幕做页」「页面对齐口播」「字幕有的要有、没有但必要也要」**，或成品为 **双人播客** 时，在拆页与 HTML 生成阶段 **必须** 遵守：

1. **逐页对齐 SRT**：用已确认的翻页边界（`wav-durations.json` 累计秒数 ↔ 大纲表）划定 **每页时间窗**，通读该窗内 **全部字幕条**；幻灯 **可见要点须覆盖该窗内关键断言**（专名、对比、因果、数字口径），不得用大段与本期论点无关的装饰或口播外硬广（如与论证无关的订阅引流）凑版面。
2. **补充材料边界**：可从同目录 **`01-script.md` / `spec.md` / 事实清单** 增补「帮助理解、不与口播矛盾」的短句或图示标签；**不得**编造未在字幕/材料中出现的性能数字或来源。
3. **图形化**：除封面与纯致谢页外，**每一内容页至少一幅信息结构图**（优先 inline SVG：流程、分层栈、对比双栏、证据链、检查清单）；图内标注须满足 `html-deck-layout` 可读下限（viewBox 内主要文字 **≥24px**，关键句 **≥28px**）。
4. **字号**：可见正文类 **≥42px**（或与 `html-deck-layout` /本地 deck 的 `--fs-card-body` 等价）；说明性辅助文字 **≥32px**。对话体口播字密度高时，应增大卡片内边距与行距，避免「缩小字号塞字」。
5. **`.wa` 锚字**：优先选 **该页时间窗内字幕中出现过的连续短语**（与口播用词一致），以利 `html-video-from-slides` 的 `timing-check` 与将来可能的 `wav-auto`；若材料用词与字幕有別，以 **字幕校对后** 的措辞为准。
6. **手改 / 自托管样式 deck**：若未走 `html-deck-layout` 仓库内 CSS、而在 `production/slides/` 自拷 `base.css` + `mobile-layout.css` 等（如 `sandbox-agent-era-mid-video` 范式），**本条 1–5 仍全部适用**；字号与图的可读下限不得因「换皮」而降低。
7. **翻页时间窗留档**：与 `wav-durations.json` 已锁定时，建议在选题 `production/timing/flip-boundaries.md`（或等价表）维护 **每页起止累计秒 + 入页首条字幕编号/摘录**；重做幻灯或大改 `.wa` 时先对照该表通读 SRT，避免漏条或与翻页脱节。
8. **禁无关主信息**：与当页时间窗字幕 **无语义交集** 的装饰性大段英文、泛泛 Slogan、占位插画不得占据主阅读区；**口播外运营话术**（如与论证无关的订阅口播）原则上不进内容页，若必须出现仅限封面一句且不挤占技术要点。

9. **Open Design（本仓克隆）**：若选题目录已提供 `production/slides/od-simple-deck.css`（由 `repos/reference/open-design/skills/simple-deck` 衍生），**必须在交付的 `presentation.html` 中 `<link>` 引入**，并为各 `section.s.slide` 添加 **`od od-light`** 或封面 **`od od-dark od-center`** 等类名，使 OD 色板与排版真实生效；仅 SKILL 正文提及 OD 而不改 HTML 视为未完成要求。
...
2. **每页时长**：30-50s 为宜；≤60s 为目标，超 60s 应尝试拆分；<20s 应与相邻段合并
...
- **无法拆分时允许超 60s**，但需在大纲（Step 3）中标注原因（如"内容不可分割"），由用户确认
...
素材目录遵循 `features/_template/content-production/` 标准布局：

```
# 幻灯在 production/slides/（标准布局）
...
```bash
node skills/html-video-from-slides/cli.js wav --project <feature>/production
```
...
node skills/html-video-from-slides/cli.js wav-auto --project <feature>/production
```
...
- [ ] `slideDurationsSec` 每项 ≤60s（超 60s 的页面需有不可拆分标注且经用户确认）
...
- [ ] `timing-check`：`node skills/html-video-from-slides/cli.js timing-check --project <production>` 不得报告 `FLIP_AT_NEXT_WA_SENTENCE_START`（否则多为「过渡句仍属上页」却对齐了下一页 `.wa` 的字幕起点）；规则见 `html-video-from-slides/SKILL.md`「翻页边界语义」
...
| `wav-durations.json` | `<feature>/production/timing/wav-durations.json` |
