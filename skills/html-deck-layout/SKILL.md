...
description: 全屏 HTML 幻灯（presentation.html，#P 1920×1080）的版式、防空白、字号与示意图区规范。与 html-video-from-slides 配套：成片截图即版式；改幻灯内容/样式时优先读本文件，减少反复口头说明。
---
...
对 **标题 + 多卡/主内容 + 底栏** 类页面使用 **`.slide-main.fill-deck`**（需在样式中定义）：

- **`.slide-main.fill-deck .slide-cards`**（或等价中间行）：`flex: 1 1 0; min-height: 0; align-items: stretch;`
- **`.slide-main.fill-deck .slide-cards > .g`**：`align-self: stretch !important; height: 100%; min-height: 0;`  
  让卡片 **拉高到标题与底栏之间**，避免「行很高、卡很矮」。

### B. 卡内如何「吃掉」高度（比单纯撑容器更有效）

每张卡建议 **列方向**：**标题区（-shrink-0）→ 正文（-shrink-0，大字）→ 示意图区（flex:1，min-height 如 200–320px）→ 要点/小结（-shrink-0）**。

- 示意图用 **全宽** 容器 + `<svg viewBox="0 0 800 340">`（或每卡一列时用 `420×260` 等），**`width/height:100%` + `preserveAspectRatio="xMidYMid meet"`**，让图随 flex 区 **放大**。
- SVG 内标题/标签字号用 **viewBox 坐标下足够大** 的字号（如 18–28），避免「图画了但像蚂蚁」。
...
## 字号档位（1920×1080，中文口播向）

**硬下限**见 `skills/html-video-from-slides/DESIGN-STANDARD.md`（如正文 **≥42px** 等）。下表为 **inline 重排版时常用区间**，勿低于 DESIGN-STANDARD。
...
**文案**：允许 **字少、句短**；字号大时更忌废话（用户不想每次强调「略删」）。

---

## 示意图（图形）原则

- **至少**：每页一个 **占面积的** 视觉锚点（整卡宽图、或每卡一块图），避免只有 chip+字。（**收口/致谢页**可无图，以大字与字标层次为主，见上文。）
- **优先全宽**：示意图放在 **正文下方、独立一层**，比「左字右窄图」更易做大。
- **与叙事一致**：对比、趋势、流程、靶心/窗口（如 6 个月）等，减少纯装饰无信息图形。

**撑满画布不必只靠 SVG**：同一期 deck 里若页页都是「深色底 + 方框流程图」，容易 **审美疲劳**。可交替使用 **纯 HTML/CSS** 的版式手段占面积：**超大标题字/数字**、**杂志分栏**、**渐变条带**、**标签云式 pill**、**引用块（大引号）**、**对角分割色块**、**等宽字模拟终端** 等；关键是 **信息密度与层次**，不是「必须有图」。
...
- [ ] 是否仍有大片 **仅网格/背景** 且无字、无图？（若有 → 先检查 fill-deck + 卡内 flex:1 图区。）
- [ ] 卡片行与底栏之间是否出现 **与内容无关的空白带**？（若有 → 检查中间行是否 flex:1 而卡片未 stretch。）
